...
public class getVINDecoderDetails {
	private static int blank = 0;
	private static int noObj = 0;

	public static void getVINDecoderDetails() throws Exception {
		...
		String[] titleStringVinDecoderWS = { "S/N", "buildDataFound:", "vin:", "modelYear:", "make:", "model:",
				...
				"styleId_01:" };
		// QA environment: Azure new url:"http://qa.istest.autodata.tech/vinid-azure/vin/"; Old used to work url: "http://lnoc-q1cp-xmw1:8080/VinId/vin/";
		String envURL = "http://qa.istest.autodata.tech/vinid-azure/vin/";
		// //DEV environment
		// String envURL = "http://lnoc-dvcp-xmw1:8080/VinId/vin/";
		//// //Prod Azure prod environment
		// String envURL = "http://znocprcpxmw1.centralus.cloudapp.azure.com:8080/VinId/vin/";
		int count = 0;
	...
	public static void getVINDecoderDetailsWS(String wsResultfile, String[] titleVinDecoderWS, String text,
		...
		double cubicInchDisplacement = 0;
		double marketClassId = 0;
		String gvwr = "";
		...
		if (text.equals("")) {
			...
			try {
				...
				// wmi obj
				JSONObject wmi = result.getJSONObject("wmi");
				String country_wmi = wmi.getString("country");
				String manufacturer = wmi.getString("manufacturer");
				String make = wmi.getString("make");
				String vehicleType = wmi.getString("vehicleType");

				// unitedStatesPattern obj
				JSONObject unitedStatesPattern = result.getJSONObject("unitedStatesPattern");
				...
				String bodyType = unitedStatesPattern.getString("bodyType");
				String driveType = unitedStatesPattern.getString("driveType");
				try {
				...
				try {
					cubicInchDisplacement = unitedStatesPattern.getDouble("cubicInchDisplacement");
				} catch (Exception ex) {
					cubicInchDisplacement = -1;
				}

				try {
					marketClassId = unitedStatesPattern.getDouble("marketClassId");
				} catch (Exception ex) {
					marketClassId = -1;
				}

				try {
					gvwr = buildData.getString("gvwr");
				} catch (Exception ex) {
					gvwr = "null";
				}

				// enginesForVin - do be done
