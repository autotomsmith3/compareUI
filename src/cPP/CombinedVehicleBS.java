...
public class CombinedVehicleBS {
	private static int blank = 0;
	private static int noObj = 0;

	public static void getCombinedVehicleBS() throws Exception {
		...
		for (int k = startToRun; k <= endToRun; k++) {// vintotal; k++) {
			...
			String getCombinedVehicleBSSavePathFile = "C:\\1\\Eclipse\\Test Results\\CombinedVehicleBS\\" + dateFolder
					+ "\\Combined-Vehicle_" + kStr + ".xls";
			String jsonpathfilename = "C:\\1\\Eclipse\\Test Results\\CombinedVehicleBS\\" + dateFolder
					+ "\\Combined-Vehicle_";// + ".json";

			String[] titleStringgetCombinedVehicleBS = { "S/N", "URL_parameter", "RequestBody", "id", "serverTime",
			...
			// // QA PUT
			// String imagePutServiceURL = "http://lnoc-q1cp-xws1:8080/vehicle-token/rest/vehicle/token"; // this is old one VehicleToken
			...
			// ******************************************************
			// ******************************************************
			String[] CombinedVehicleBodyParameters = new String[dataLength + 1];
			int datasize = CombinedVehicleBodyParameters.length;
			for (int i = 0; i < datasize - 1; i++) {
				count++;
				String iStr = Integer.toString(i + 1);

				String bodyDatafile = "C:\\1\\Eclipse\\Test Results\\CombinedVehicleBS\\TestData\\VehicleTokenBody"
						+ iStr + ".txt"; // # group number testing

				CombinedVehicleBodyParameters[i] = getFile(bodyDatafile); // ok
				// CombinedVehicleBodyParameters[i] = LoadFileToString(bodyDatafile); //failed
				// String getSourceCode(String prefix, String url)

				// PUT New MS Image endpoint
				String jsonTextFrgetImageWS = "";
				try {
					jsonTextFrgetImageWS = com_libs.getSourceCodeJson(CombinedVehicleBodyParameters[i],
				...
				getCombinedVehicleBSDetails(getCombinedVehicleBSSavePathFile, titleStringgetCombinedVehicleBS,
						jsonTextFrgetImageWS, imagePutServiceURL, CombinedVehicleBodyParameters[i], count);
			}
		}
	}

	public static void getCombinedVehicleBSDetails(String wsResultfile, String[] titleString, String text,
					...
					for (int i = 0; i < size; i++) {
						...
						try {
							...
							for (int j = 0; j < tokenssize; j++) {
								...
								try {
									buildDataSource = tokensArray.getJSONObject(j).getString("buildDataSource");
								} catch (Exception ex) {
								...
								try {
									JSONArray additionalGVUIDSArray = tokensArray.getJSONObject(i)
											.getJSONArray("additionalGVUIDs");
